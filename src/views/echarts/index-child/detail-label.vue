<template>
  <div class="detail-wrapper">
    <!-- 上方标签栏 -->
    <div class="label-scroll" ref="labelWrapper"  style="opacity: 0">
...
export default {
  ...
  methods: {
    initChart() {
      ...
      this.chart.setOption({
        ...
        tooltip: {
          ...
          formatter: `<div style="transform: translate3d(-50%, -50%, 0); text-align: center"><span class="" style="font-size: 20px;">{b}</span><br/><span class="digital-number" style="font-size: 28px;">{d}%</span><br/><span class="digital-number">{c}个</span></div>`,
          backgroundColor: "transparent",
          textStyle: {
            fontSize: 26,
          },
          ...
          tooltip: {
            ...
            formatter: function (name) {
              ...
              if (_index > -1 && name.name !== " ") {
                const _allNumber = _this.totalSum;
                console.log()
                return `<div style="transform: translate3d(-50%, -50%, 0); text-align: center"><span>${name.name}</span><br/><span class="digital-number" style="font-size: 28px">${
                  _allNumber === 0
                    ? 0
                    : ((_this.itemValueDetail[_index].value * 100) / _allNumber)
                        .toFixed(2)
                        .endsWith("0")
                    ? ((_this.itemValueDetail[_index].value * 100) / _allNumber)
                        .toFixed(1)
                        .endsWith("0")
                      ? (
                          (_this.itemValueDetail[_index].value * 100) /
                          _allNumber
                        ).toFixed(0)
                      : (
                          (_this.itemValueDetail[_index].value * 100) /
                    ...
                    : ((_this.itemValueDetail[_index].value * 100) / _allNumber).toFixed(
                        2
                      )
                }%</span><br/><span class="digital-number">${
                  _this.itemValueDetail[_index].value
                }个</span></div>`;
              } else {
                return `<div style="transform: translate3d(-50%, -50%, 0); text-align: center"><span class="digital-number" style="font-size: 28px;">${_this.itemValueDetail[_index].value}个<span></div>`;
              }
            },
            backgroundColor: "transparent",
            textStyle: {
    ...
    selectItem(index) {
      ...
      this.currentSelectedItemAllNumber = this.labelData[index].all
        ? this.labelData[index].all
        : 0;
        ...
        if (document.querySelector(".chart-graph").lastChild) {
          if (document.querySelector(".chart-graph").lastChild.innerHTML === "") {
            ...
            if (document.querySelector(".chart-graph").lastChild.style.zIndex > 1) {
              ...
              document.querySelector(
                ".chart-graph"
              ).lastChild.innerHTML = `<div style="transform: translate3d(-50%, -50%, 0); text-align: center"><span class="digital-number" style="font-size: 28px;">${
                this.currentSelectedItemAllNumber === 0
                  ? 0
                  : (
                      (this.itemValueDetail[0].value * 100) /
